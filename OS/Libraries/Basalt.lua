...
return adb(_bc(bbc or bc(),abc))end end;setmetatable(baa,c_a)return baa end end
...
if(_ca)then cba.parent:addEvent(dba,cba)end end end;if not(dd)then dd=true;return true end end}cc.__index=cc;return cc end end;aa["objects"]={}
...
(_ba~="Terminated")then
local aba=baa:sendEvent("program_error",baa,_ba)
if(aba~=false)then error("Basalt Program - ".._ba)end end;if(__a:getStatus()=="dead")then
baa:sendEvent("program_done",baa)end end
...
a_a.parent:addEvent("mouse_drag",a_a)a_a.parent:addEvent("mouse_scroll",a_a)end}return setmetatable(__a,_c)end end
...
d_a.parent:addEvent("mouse_up",d_a)d_a.parent:addEvent("mouse_scroll",d_a)end end}return setmetatable(c_a,_c)end end;aa["libraries"]={}
aa["libraries"]["images"]=function(...)local function ab(db)return
paintutils.loadImage(db),"nfp"end
local function bb(db)
local _c=fs.open(db,"rb")
local ac=load("return ".._c.readAll())()_c.close()if(ac~=nil)then return ac,"bimg"end end
